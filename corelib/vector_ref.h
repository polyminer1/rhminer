...
public:
	...
	template <class _T2> explicit operator vector_ref<_T2>() const { RHMINER_ASSERT(m_count * sizeof(_T) / sizeof(_T2) * sizeof(_T2) / sizeof(_T) == m_count); return vector_ref<_T2>(reinterpret_cast<_T2*>(m_data), m_count * sizeof(_T) / sizeof(_T2)); }
	...
	_T& operator[](size_t _i) { RHMINER_ASSERT(m_data); RHMINER_ASSERT(_i < m_count); return m_data[_i]; }
	_T const& operator[](size_t _i) const { RHMINER_ASSERT(m_data); RHMINER_ASSERT(_i < m_count); return m_data[_i]; }
